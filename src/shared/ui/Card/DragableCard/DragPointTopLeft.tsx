import { resizeFromTopLeft } from "../../../lib/dragable/resize";
...
export default function DragPointTopLeft({ animationControl }: Props) {
  ...
  const handleMouseDown = (event: MouseEvent<HTMLDivElement>) => {
    ...
    const { top, left } = motion.getBoundingClientRect();
    const currentY = top;
    const currentX = left;
    ...
    const moveTo = (e: globalThis.MouseEvent) => {
      const { width, height, top, left } = resizeFromTopLeft({
        currentY,
        currentX,
        currentHeight,
        currentPageX,
      ...
      animationControl.set({ y: top, x: left });
    };
